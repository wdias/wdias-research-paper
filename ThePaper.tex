...
\dbc{From Table II to V it seems we separated out timeseries and grid requests. It need to be mentioned in this sub-section to avoid any confusion.}
...
\dbc{How comeheavy writing happens when retrieving data? Shouldn't it happen during ingestion. Or is it because we are running data preprocessing at the time of request? This need to be clearly explained.}
...
Latency to insert grid timeseries data is relatively low compared to scalar or vector inserts as grid data are handle asynchronously. When the number of grid files doubled, latency also doubles. \db{Yet, \acrshort{wdias} was able to handle the all the incoming requests.}
